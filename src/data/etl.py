...
def run_etl():
    ...
    df = df.merge(
        dim_credit_history[
            ["credit_score", "payment_history", "previous_defaults", "credit_history_id"]
        ],
    ...
    try:
        ...
        dim_customers_load.to_sql(
            "dim_customers", engine, if_exists="append", index=False, method="multi", chunksize=500
        )
        print("  - Loaded dim_customers")

        dim_geography_load.to_sql(
            "dim_geography", engine, if_exists="append", index=False, method="multi", chunksize=500
        )
        print("  - Loaded dim_geography")

        dim_credit_history_load.to_sql(
            "dim_credit_history",
            engine,
            if_exists="append",
            index=False,
        ...
        fact_loans_load.to_sql(
            "fact_loans", engine, if_exists="append", index=False, method="multi", chunksize=500
        )
