...
def create_domain_features(df):
    ...
    # === RISK INDICATORS ===
    df["high_dti_flag"] = (df["debt_to_income_ratio"] > 0.43).astype(
        int
    )  # 43% is standard threshold
    ...
    df["job_stability"] = df["years_at_current_job"] / (
        df["age"] - 18 + 1
    )  # % of career at current job
    ...
    df["affordability_index"] = (df["income"] - df["loan_amount"] * 0.05) / (
        df["income"] + 1
    )  # Assume 5% annual payment
...
def preprocess_data(df):
    ...
    print(
        f"  ✓ Classes: {dict(zip(label_encoder.classes_, y.value_counts().sort_index().tolist()))}"
    )
